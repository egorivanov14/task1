...
public class CustomIntArrayParserImpl implements CustomIntArrayParser {
  ...
  @Override
  public int[] parseToIntArray(String lineOfInts) throws CustomIntArrayParserException {
    ...
    if (lineOfInts != null && !lineOfInts.isEmpty()) {
      if (validator.isLineOfIntsValid(lineOfInts)) {
        ...
        for (int i = 0; i < intArray.length; i++) {
          if (validator.isElementValid(strNumbers[i], pattern)) {
            ...
            try{
              fileWriter.writeWrongValues(strNumbers[i]);
            }catch (CustomIntArrayWriterException e){
              throw new CustomIntArrayParserException("Invalid parsing.", e);
            }
          }
        }
      ...
      throw new CustomIntArrayParserException("Failed to parse line to int array. Array is null or empty.");
    }
  }
}